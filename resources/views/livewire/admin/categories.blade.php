<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-sm text-slate">Total {{ $categories->count() }} kategori</p>
        </div>
        <button
    ...
    @if($categories->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($categories as $category)
                <div class="bg-white rounded-xl border border-slate-200 p-5 hover:border-navy/30 transition-all">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate">{{ $category->products_count }} produk</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-1">
                            <button wire:click="openModal({{ $category->id }})" class="p-2 hover:bg-slate-100 rounded-lg transition-colors">
                                <svg class="w-4 h-4 text-slate" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </button>
                            <button wire:click="delete({{ $category->id }})" wire:confirm="Yakin hapus kategori ini?" class="p-2 hover:bg-error/10 rounded-lg transition-colors">
    ...
    @else
        <div class="bg-white rounded-xl border border-slate-200 p-12 text-center">
            ...
            <p class="text-slate">Belum ada kategori</p>
            <button wire:click="openModal()" class="mt-4 text-sm text-sage hover:underline">Tambah kategori pertama</button>
        </div>
    @endif
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" wire:click="closeModal" wire:click.self="closeModal">
            ...
            <div class="relative bg-white rounded-xl shadow-lg w-full max-w-sm p-6" wire:click.stop>
                ...
                <form wire:submit="save">
                    <div>
                        <label class="block text-sm font-medium text-navy mb-1.5">Nama Kategori</label>
                        <input type="text" wire:model="name" class="w-full h-11 px-4 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-2 focus:border-navy focus:ring-2 focus:ring-navy/10 @error('name') border-error @enderror" placeholder="Contoh: Makanan, Minuman, Appetizer">
